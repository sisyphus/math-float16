...
=head1 DESCRIPTION
 ...
 Perform  _Float16 (brain float) arithmetic  in perl.
...
=head1 SYNOPSIS
 ...
 NOTE !!!!!!! :
  ...
  once to a  _Float16. If you first assign that value to something else
  (eg an NV), and then assign it to a Math::Float16 object, the
  value can be rounded twice. In the following example the value is
  firstly rounded to an NV, and secondly from the NV to the 11-bit
  precision Math::Float16 object. EXAMPLE (assuming that the NV
  is of type 'double'):

   my $pv = '1.96500000000000014e2';
   my $nv =  1.96500000000000014e2 ;
   print Math::Float16->new($pv); # 1.97e2 (rounded only once)
   print Math::Float16->new($nv); # 1.96e2 (rounded twice)

  Mostly it won't matter, but we see here that it sometimes can.

 The  _Float16 has only 8 bits of precision.
...
=head1 OUTPUT FUNCTIONS
   ...
   $string = unpack_f16_hex($obj);
    ...
    For example, unpack denorm min and +Inf:
      print unpack_f16_hex(Math::Bfloat->new(0);          # 0000
      print unpack_f16_hex(Math::Bfloat->new('-0');       # 8000
      print unpack_f16_hex(Math::Bfloat->new(2) ** -133); # 0001
      print unpack_f16_hex(Math::Bfloat->new(2) ** 128);  # 7F80 (+inf)
...
=head1 MISCELLANEOUS FUNCTIONS
   ...
   $MPFR_OBJECT = f16_to_MPFR($obj);
    ...
    The instructions "print $obj;" and "print f16_to_MPFR($obj);"
